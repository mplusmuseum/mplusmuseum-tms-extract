...
exports.index = async (req, res) => {
  ...
  const processFilename = path.join(rootDir, 'imports', 'Exhibitions', tms, 'process', subFolder, `${id}.json`)
  const processedFilename = path.join(rootDir, 'imports', 'Exhibitions', tms, 'processed', subFolder, `${id}.json`)
  const perfectFilename = path.join(rootDir, 'imports', 'Exhibitions', tms, 'perfect', subFolder, `${id}.json`)
  const xmlFilename = path.join(rootDir, 'imports', 'Exhibitions', tms, 'xml', subFolder, `${id}.xml`)
  ...
  //  Grab the query used to ask for an exhibitions
  const queries = new Queries()
  const searchFilter = `(id: ${id})`
  const query = queries.get('exhibitionLarge', searchFilter)
  ...
  return res.render('search/exhibitions', req.templateValues)
}
