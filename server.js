...
if (!fs.existsSync(path.join(rootDir, '.env'))) {
  ...
  let port = prompt('port number (4001): ')

  if (port === '') port = 4001
  port = parseInt(port)
  if (isNaN(port) || port < 0 || port > 49151) {
    console.log('Port must be between 0-49151, setting port to 4001'.alert)
    port = 4001
  }
...
//  If we have elastic search setup then we use that for sessions,
//  otherwise we use the file system
